...
public class GUI {
	...
	public GUI() {
		...
		//Get information on service by using run method from Discovery class and store in serviceInfo object
		serviceInfo1  = jmDNSServiceDiscovery.run("_grpc._tcp.local");
		serviceInfo2  = jmDNSServiceDiscoveryTransport.run("_transport._tcp.local");
		//get port from service Information
	...
	private void initialize() {
		...
		JLabel lblNewLabel_1 = new JLabel(" *UNARY RPC* Please enter the Air Quality Index or your area ");
		panel_service_1.add(lblNewLabel_1);
		
		JLabel lblNewLabel_2 = new JLabel(" *BIRDIRECTIONAL RPC* Please enter the outbound and return distance for Bus trip in KM in the boxes ");
		panel_service_2.add(lblNewLabel_2);

		JLabel lblNewLabel_3 = new JLabel(" *SERVER STREAMING RPC* Please enter a number representing the distance travelled on Rail in KM in the box ");
		panel_service_3.add(lblNewLabel_3);

		JLabel lblNewLabel_4 = new JLabel(" *CLIENT STREAMING RPC* Enter the distance travelled by car in the first box and your cars consumption per liter in the second");
		...
		JButton btnSend = new JButton("Send Air Quality Index to AirQualityServer");

		JButton btnSend2 = new JButton("Send numbers to TransportServer");

		JButton btnSend3 = new JButton("Send the distance travelled to the TransportServer");

		JButton btnSend4 = new JButton("Send to TransportServer ");
			...
			public void actionPerformed(ActionEvent b) {
				...
				StreamObserver<busResponse> responseObserver = new StreamObserver<busResponse>() {
					...
					public void onNext(busResponse value) {
						...
						textResponse2.append("Your CO2 Emissions will be "+value.getEmissions()+"KG for travelling on a "+value.getMessage()+"\n");
					}
		...
		btnSend4.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent b) {
				...
				try {
					...
					textResponse4.append("Your Emissions arebdfenj");
					e1.printStackTrace();
				}
			}
			});
